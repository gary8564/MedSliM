...
def _resolve_pooling_target(
    ...
    if regional_tokens > 0 and resolved == "raw":
        raise ValueError(
            "pooling_target='raw' is not supported with flattened regional tokens "
            "(regional_tokens > 0). Use pooling_target='post_encoder' or 'post_embed'."
        )
...
class Cobra(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        Args:
            ...
            regional_tokens: Number of regional crop tokens per slice for flattened
                regional-token pretraining. 0 keeps global-only CLS. When > 0,
                inputs are expected to contain 1 + regional_tokens tokens per slice
                (global token plus regional crops), and Cobra flattens
                [B, num_slices, 1+regional_tokens, E] to
                [B, num_slices * (1+regional_tokens), E] before the sequence encoder.
                This does not use a WithinSliceAggregator.
        ...
        if regional_tokens < 0:
            raise ValueError(f"regional_tokens must be >= 0, got {regional_tokens}.")
    ...
    # ------------------------------------------------------------------ #
    # Public use-case preparation: each returns slice features [B, D, E]
    # ready for the sequence encoder.
    # ------------------------------------------------------------------ #
    ...
    def _forward(
        ...
        **_,
        ...
        # Prepare slice features [B, num_slices, embed_dim] per public use case:
